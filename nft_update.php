...
if(!empty($title)){
        ...
        $query = "UPDATE nft SET title=?, description=?, current_price=?, logo=? WHERE id=?";
        ...
        die;} 
            ...
            header("location: nft_edit.php?id=$id");
        die;
        }
    }
    else{
        // uporabnik Ni nalozil logotipa
        $query = "UPDATE nft SET title=?, description=?, current_price=? WHERE id=?";
        $stmt = $pdo->prepare($query);
